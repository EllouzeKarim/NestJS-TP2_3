...
export class NewtodoService {
  ...
  getTodos(queryParams: SearchQueryDto) {
    ...
    return this.todoRepository.find({
      where: [
        ...
        { name: Like(`%${queryParams.criteria}%`) },
        { description: Like(`%${queryParams.criteria}%`) },
      ],
    });
  }
  ...
  async getTodosv2(queryParams: SearchQueryDto): Promise<TodoEntity[]> {
    ...
    qb.where(
      new Brackets((q) => {
        q.where(
          'todos.name LIKE :criteria OR todos.description LIKE :criteria',
          { criteria: `%${criteria}%` },
        );
      }),
  ...
  async countTodos() {
    ...
    for (const statut of statuts) {
      counts[statut] = await this.todoRepository.count({
        where: { status: statut },
      });
    }
    return counts;
